...
for infile in args:
    ...
    #             footprint[i] = [footprint[i][1], footprint[i][0]]
    #         center = np.mean(footprint, axis=0)
    #         width = center[0] - footprint[0][0]
...
for path in paths:
   ...
   #  if circle:
#         print circle_pos,circle_rad
#         annim.Skypolygon('ellipse', cpos=circle_pos, major=2*circle_rad, minor=2*circle_rad, units='arcmin', ls='dashed', fc=(0.9,0.9,0.1,0.01))
    ...
    for match in matches:
        ...
        for circle in circles:
            if circle[0] == s:
                ...
                print circle[1]
                print pos
                annim.Skypolygon('ellipse', cpos=pos, major=2*circle[1], minor=2*circle[1], units='arcmin', ls='dotted', fc='m', fill=False)
                fs = 8
    ...
    kpc_ruler = annim.Ruler(x1=100, y1=1900, rulersize=0.05, lambda0=0.0,
                      step=0.025239, units='deg', fliplabelside=True, fun=lambda x: x/0.025239 * 10, fmt="%d kpc", addangle=90, size=4)
    annim.plot()
    ...
    # pixels = w.wcs_world2pix(footprint, 0)
    # poly = Polygon(pixels)
    # x,y = poly.exterior.xy
    # plt.imshow(path[0].data, origin='lower')
