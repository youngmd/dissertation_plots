...
eta[i]<-beta.0+beta.1*x_true[i]
mu[i]<-max(0.0001, 1/eta[i] + errorY[i])
...
# # 3.Prediction of current data
  ...
  #muTrue[i]<-max(0.001,min(20,etaTrue[i]))
  #prediction.norm[i]~dgamma(muTrue[i]^2 / sigma^2, muTrue[i] / sigma^2)
}
# Fit<-sum(D[1:N])
# New<-sum(DNew[1:N])
...
for (j in 1:M){
    ...
    mux[j]<-max(0.0001, 1/etax[j])
    prediction.Normx[j]~dgamma(mux[j]^2 / sigma^2, mux[j] / sigma^2)
  }
}"
...
inits<-function(){list(beta.0=rnorm(1,0,0.1),beta.1=rnorm(1,0,0.1),sigma=runif(1,0.1,50))}
...
jags.gamma <- autorun.jags(method="rjparallel",
...
jagssamples.gamma<-extend.jags(jags.gamma, drop.monitor=c('beta.0','beta.1','sigma','dic'), add.monitor=c('prediction.Normx'), summarise=FALSE)
...
pred.Normerrx<-summary(as.mcmc.list(jagssamples.gamma, vars="prediction.Normx"),quantiles=c(0.005,0.025,0.25,0.5,0.75,0.975, 0.995))
...
inits <- list('beta.0'=summary$summaries['beta.0','Mean'], 'beta.1'=summary$summaries['beta.1','Mean'],sigma=summary$summaries['sigma','Mean'])
...
dicsamples.gamma <- dic.samples(jags.DIC, params, n.iter = 15000,type="pD")
