...
y[i]~dnorm(mu[i],tau)
mu[i]<- exp(beta.0+beta.1*x_true[i]) + errorY[i]
...
inits<-function(){list(beta.0=rnorm(1,0,0.1),beta.1=rnorm(1,0,0.1),sigma=runif(1,0.1,20))}
...
S.G2<-ggs(ssamples.n,family=c("tau"))
...
jagssamples.norm<-extend.jags(jags.norm, drop.monitor=c('beta.0','beta.1','tau','dic'), add.monitor=c('prediction.Normx'), summarise=FALSE)
...
inits <- list('beta.0'=summary$summaries['beta.0','Mean'], 'beta.1'=summary$summaries['beta.1','Mean'],sigma=1/(sqrt(summary$summaries['tau','Mean'])))

jags.DIC <- jags.model(
  data = jags.data, 
  inits = inits, 
  textConnection(model.norm),
  module = c('glm'),
  n.chains = 3,
  n.adapt=2000
)
...
dicsamples.norm <- dic.samples(jags.DIC, params, n.iter = 15000,type="pD")
