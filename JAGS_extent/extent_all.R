...
GCS = subset(GCS, !is.na(Extent_err)) 
GCS = subset(GCS, Extent_err > 0 ) 
...
calcD <- function(Dstats, pred, D2){
  DIC <- sum(D2$deviance) + sum(D2$penalty)
  Dstats2 <- data.frame('pred'=pred, 'DIC'=DIC)
  Dstats <- rbind(Dstats, Dstats2)
  return(Dstats)
}

Dstats <- data.frame('pred'=character(), 'DIC'=double())

ylabel <- expression( Extent~of~GC~System)
N_low<-GCS$Extent-GCS$Extent_err
N_low[N_low<0]<-0
GCS$Type <- factor(GCS$group_type, levels = c("E", "Spiral", "S0"))
# GCS$y <- log(GCS$Extent)
# GCS$ylow <- GCS$y - log(GCS$Extent - GCS$Extent_err)
# GCS$yhigh <- log(GCS$Extent + GCS$Extent_err) - GCS$y
# GCS$y_err <- GCS$ylow
GCS$y <- GCS$Extent
GCS$ylow <- GCS$Extent_err
GCS$yhigh <- GCS$Extent_err
GCS$y_err <- GCS$Extent_err
...
# GCS$sig <- log10(GCS$sig_e)
# GCS$sig_err <- log10(GCS$sig_e + GCS$sig_e_err) - GCS$sig

# GCS$R <- log10(GCS$R_e)
# GCS$R_err <- log10(GCS$R_e + GCS$R_e_err) - GCS$R

df <- data.frame(Type = GCS$Type, y = GCS$y, y_err = GCS$y_err, ylow = GCS$ylow, yhigh = GCS$yhigh)

summaries <- list()

#preds <- c("L_K","L_V","Mdyn","mass","R","sig")
#labs <- c("L[K]","L[V]","M[dyn]","M[*]","R[eff]","sigma[e]")
preds <- c("Mdyn","mass","L_K","L_V","R_e","sig_e")
labs <- c("M[dyn]","M[*]","L[K]","L[V]","R[eff]","sigma[e]")
...
for (p in preds) {
    ...
    cl <- makeCluster(3)
    source('gamma.R',print.eval=TRUE)
    summaries[[p]] = summary
    stopCluster(cl)
    gc()
    df$xlow <- df$up_x
    df$xhigh <- df$up_x
    s <- labs[[pind]]
    g1 <- plotfit(df, pred, s, ylabel)
    ggsave(paste(datapath,p,'_gfit.pdf',sep=''), g1, width=8, height=8, device=cairo_pdf)
    ggsave(paste(datapath,p,'_gdiag.pdf',sep=''), postplots, width=8, height=8, device=cairo_pdf)
    ggsave(paste(datapath,p,'_gtrace.pdf',sep=''), traceplots, width=8, height=8, device=cairo_pdf)
    #DIC = summary$dic$dic
    # bod2[counter,]$DIC = DIC

    Dstats <- calcD(Dstats, p, dicsamples.gamma)
    rm(postplots)
    rm(traceplots)
    rm(jags.gamma)
    rm(dicsamples.gamma)
...
save(summaries, file='summaries_lnorm.Robj')
save(Dstats, file="Dstats_lnorm.Robj")
# xlabel <- expression(sigma~~(km/s))
# GCS$x <- GCS$sig_e
# GCS$x_err <- GCS$sig_e_err
# GCS$xlow <- GCS$sig_e_err
# GCS$xhigh <- GCS$sig_e_err
# source('norm.R', print.eval=TRUE)
# pred.Norm2errx$x <- pred.Norm2errx$x2
# g1 <- 
# ggsave(paste(datapath,'Sig.pdf'), g1, width=8, height=8, device=cairo_pdf)
# Dstats <- calcD(Dstats, 'Sig', Dispersion, dicsamples.norm)

# xlabel <- expression(M[V])
# GCS$x <- GCS$M_V
# GCS$x_err <- GCS$M_V_err
...
# Dstats <- calcD(Dstats, 'MV', Dispersion, dicsamples.norm)

# xlabel <- expression(log~R[e]~~(kpc))
# GCS$x <- GCS$R_e
# GCS$x_err <- GCS$R_e_err
...
# Dstats <- calcD(Dstats, 'R_e', Dispersion, dicsamples.norm)

# GCS$x <- GCS$mass
# GCS$x_err <- GCS$mass_err
...
# Dstats <- calcD(Dstats, 'M_stellar', Dispersion, dicsamples.norm)

# GCS$x <- GCS$Mdyn
# GCS$x_err <- GCS$Mdyn_err
# GCS$xlow <- GCS$Mdyn_err
# GCS$xhigh <- GCS$Mdyn_err
# source('norm2.R', print.eval=TRUE)
...
# Dstats <- calcD(Dstats, 'M_dyn', Dispersion, dicsamples.norm)
...
# ggsave(paste(datapath,'Extent_plots.pdf'), splot, width=8, height=8, device=cairo_pdf)
...
#     labs(x="",
#             y="",
#             title=expression(DIC~of~Radial~Extent~Predictors)) +
#     theme(axis.text.x =
...
#     coord_flip() +
#     scale_x_discrete(labels=parse(text=c("Mass[stellar]","Mass[dyn]"))) +
#     scale_fill_economist() 

# CairoPDF(paste(datapath,'Extent_all_DIC.pdf',sep=''),height=5,width=9)
# p
# dev.off()
...
#     labs(x="",
#             y="",
#             title=expression(Dispersion~of~Radial~Extent~Predictors)) +
#     theme(axis.text.x =
#          element_text(size  = 10,
#                       angle = 0,
#                       hjust = 1,
...
# CairoPDF(paste(datapath,'Extent_all_disp.pdf',sep=''),height=5,width=9)
# p2
# dev.off()
