...
mu[i]<- exp(beta.0+beta.1*x_true[i]+beta.2*z_true[i]) + errorY[i]

errorY[i]~dnorm(0, 1/(erry[i]^2))# Errors in y
...
#   muTrue[i]<-beta.0+beta.1*x_true[i]+beta.2*z_true[i]
...
# # 4.Prediction new data
  ...
  for (j in 1:M){
    mux[j]<-exp(beta.0+beta.1*dfx[j]+beta.2*dfz[j])
# #  log(mux[j])<-max(-20,min(20,etax[j]))
    prediction.Normx[j]~dnorm(mux[j],tau)
  }
}"

inits<-function(){list(beta.0=rnorm(1,0,0.1),beta.1=rnorm(1,0,0.1),beta.2=rnorm(1,0,0.1),sigma=runif(1,0.1,5))}
...
params <- c("beta.0","beta.1","beta.2","tau")
...
jags.norm <- autorun.jags(method="rjparallel",
...
summary<-extend.jags(jags.norm, summarise=TRUE)
jagssamples.norm<-extend.jags(jags.norm, drop.monitor=c('beta.0','beta.1','beta.2','tau'), add.monitor=c('prediction.Normx'), summarise=FALSE)

pred.Normerrx<-summary(as.mcmc.list(jagssamples.norm, vars="prediction.Normx"),quantiles=c(0.005,0.025,0.25,0.5,0.75,0.975, 0.995))
...
# jagssamples.n3 <- as.mcmc.list(summary)

# # jagssamples.nb <- as.mcmc.list(jags.norm )
# # summary<-extend.jags(jags.norm,drop.monitor=c("PRes","x_true","z_true","Fit","New","prediction.Normx"), summarise=TRUE)
...
jagssamples.n2 <- as.mcmc.list(summary)

S.G1<-ggs(jagssamples.n2,family=c("beta"))
S.G2<-ggs(jagssamples.n2,family=c("tau"))
...
# Dispersion = sum(Pres^2)/(N-4)# beta.0, beta.1, beta.2, tau, 4 parameters


# jags.DIC <- jags.model(
#   data = jags.data, 
#   inits = inits1, 
#   textConnection(model.norm2),
#   n.chains = 3,
#   n.adapt=2000
# )
