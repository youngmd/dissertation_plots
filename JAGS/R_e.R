...
Re[i]~dnorm(Re_true[i],1/err_Re[i]^2);

errorN[i]~dbin(0.5,2*errN_GC[i])

eta[i]<-beta.0+beta.1*Re_true[i]
...
# Prediction
etaTrue[i]<-beta.0+beta.1*Re_true[i]
...
params <- c("beta.0","beta.1","size","PRes","Re_true","Fit","New","prediction.NBx")
...
summary<-extend.jags(jags.neg,drop.monitor=c("PRes","Re_true","Fit","New","prediction.NBx"), summarise=TRUE)
...
#Re_true<-summary(as.mcmc.list(jags.neg,vars="Re_true"),quantiles=0.5)
...
pred.NB2errx<-data.frame(Rex=Rex,mean=pred.NBerrx$statistics[,1],lwr1=pred.NBerrx$quantiles[,3],lwr2=pred.NBerrx$quantiles[,2],lwr3=pred.NBerrx$quantiles[,1],upr1=pred.NBerrx$quantiles[,5],upr2=pred.NBerrx$quantiles[,6],upr3=pred.NBerrx$quantiles[,7])
...
# g2<-ggplot(GCS,aes(x=R_e,y=N_GC))+
#   geom_ribbon(data=pred.NB2errx,aes(x=Rex,y=mean,ymin=lwr1, ymax=upr1), alpha=0.45, fill="blue4") +
#   geom_ribbon(data=pred.NB2errx,aes(x=Rex,y=mean,ymin=lwr2, ymax=upr2), alpha=0.35, fill="blue4") +
#   geom_ribbon(data=pred.NB2errx,aes(x=Rex,y=mean,ymin=lwr3, ymax=upr3), alpha=0.25, fill="blue4") +
...
#   geom_errorbarh(aes(colour=Type,xmin=R_e-R_e_err, xmax=R_e+R_e_err),alpha=0.7,height=0.05)+
#   geom_line(data=pred.NB2errx,aes(x=Rex,y=mean),colour="gray25",linetype="dashed",size=0.8)+
#   scale_y_continuous(trans='asinh',breaks=c(0,10,100,1000,10000,100000,1000000),labels=c("0",expression(10^1),expression(10^2),expression(10^3),expression(10^4),expression(10^5),expression(10^6)))+
#   scale_x_log10() +
#   scale_colour_gdocs()+
...
#   ylab(ylabel)+
#   xlab(expression(log(R[e])~~(kpc)))

# CairoPDF(paste(datapath,'Re.pdf',sep=''),height=8,width=9)
# g2
# dev.off()
