...
df <- data.frame(y = GCS$Nblue, y_err = GCS$Nblue_err)

summaries <- list()
outputs <- list()
preds <- c("Mdyn","L_K")
# bod2 <- expand.grid(x1 = preds, x2 = preds)
# bod2$DIC <- 0
# counter = 0
...
save(summaries, file='bluesummaries_autorun.Robj')
save(outputs, file='blueoutputs_autorun.Robj')
save(Dstats, file='bluedstats_autorun.Robj')

# save(bod2, file='bod2.Robj')
# write.table(bod2, 'bod2.dat')
