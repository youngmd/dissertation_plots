#dim(GCS)
N_err<-GCS$N_GC_err
err_mass<-GCS$mass_err
N = nrow(GCS)

######## NB with errors ########################################################
massx = seq(from = 0.95 * min(GCS$mass), 
            to = 1.05 * max(GCS$mass), 
            length.out = 500)
...
mass[i]~dnorm(mass_true[i],1/err_mass[i]^2);

errorN[i]~dbin(0.5,2*errN_GC[i])

eta[i]<-beta.0+beta.1*mass_true[i]
...
# Prediction
etaTrue[i]<-beta.0+beta.1*mass_true[i]
...
params <- c("beta.0","beta.1","size","PRes","mass_true","Fit","New","prediction.NBx")
...
summary<-extend.jags(jags.neg,drop.monitor=c("PRes","mass_true","Fit","New","prediction.NBx"), summarise=TRUE)
...
pred.NB2errx<-data.frame(massx=massx,mean=pred.NBerrx$statistics[,1],lwr1=pred.NBerrx$quantiles[,3],lwr2=pred.NBerrx$quantiles[,2],lwr3=pred.NBerrx$quantiles[,1],upr1=pred.NBerrx$quantiles[,5],upr2=pred.NBerrx$quantiles[,6],upr3=pred.NBerrx$quantiles[,7])
...
# g2<-ggplot(GCS,aes(x=mass,y=N_GC))+
#   geom_ribbon(data=pred.NB2errx,aes(x=massx,y=mean,ymin=lwr1, ymax=upr1), alpha=0.45, fill="gray") +
#   geom_ribbon(data=pred.NB2errx,aes(x=massx,y=mean,ymin=lwr2, ymax=upr2), alpha=0.35, fill="gray") +
#   geom_ribbon(data=pred.NB2errx,aes(x=massx,y=mean,ymin=lwr3, ymax=upr3), alpha=0.25, fill="gray") +
...
#   geom_errorbarh(aes(colour=Type,xmin=mass-GCS$mass_err, xmax=mass+mass_err),alpha=0.7,height=0.05)+
#   geom_line(data=pred.NB2errx,aes(x=massx,y=mean),colour="gray25",linetype="dashed",size=1.2)+
...
#   ylab(ylabel)+
#   xlab(expression(log~M[stellar]/M['\u0298']))

# CairoPDF(paste(datapath,'mass.pdf',sep=''),height=8,width=9)
# g2
# dev.off()
