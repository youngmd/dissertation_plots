...
# preds <- c("lg_M_B","Mdyn","mass","R_e","L_K","L_V","sig_e")
...
#save(summaries, file='MBHsummaries.Robj')
load(file='MBHoutputs.Robj')

xlabel <- expression(log~M[dyn]/M['\u0298'])
GCS$x <- GCS$Mdyn
GCS$xlow <- GCS$Mdyn_err
GCS$xhigh <- GCS$Mdyn_err
g1 <- plotfit(GCS, outputs$Mdyn, xlabel, ylabel)

xlabel <- expression(log~M[BH]/M['\u0298'])
GCS$x <- GCS$lg_M_B
GCS$xlow <- GCS$lg_M_B_err_low
GCS$xhigh <- GCS$lg_M_B_err_high
g2 <- plotfit(GCS, outputs$lg_M_B, xlabel, ylabel)

xlabel <- expression(log~M["*"]/M['\u0298'])
GCS$x <- GCS$mass
GCS$xlow <- GCS$mass_err
GCS$xhigh <- GCS$mass_err
g3 <- plotfit(GCS, outputs$mass, xlabel, ylabel)

xlabel <- expression(R[eff])
GCS$x <- GCS$R_e
GCS$xlow <- GCS$R_e_err
GCS$xhigh <- GCS$R_e_err
g4 <- plotfit(GCS, outputs$R_e, xlabel, ylabel)

xlabel <- expression(L[K])
GCS$x <- GCS$L_K
GCS$xlow <- GCS$L_K_err
GCS$xhigh <- GCS$L_K_err
g5 <- plotfit(GCS, outputs$L_K, xlabel, ylabel)

xlabel <- expression(L[V])
GCS$x <- GCS$L_V
GCS$xlow <- GCS$L_V_err
GCS$xhigh <- GCS$L_V_err
g6 <- plotfit(GCS, outputs$L_V, xlabel, ylabel)

splot <- arrangeGrob(g1, g2, g3, g4, g5, g6, nrow=3, ncol=2)
ggsave(paste('Figures/Ngc_wmbh_plots.pdf'), splot, width=8, height=10.5, device=cairo_pdf)
