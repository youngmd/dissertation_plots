...
samples.nb <- extend.jags(jags.neg, drop.monitor=c("beta.0","beta.1","size"), add.monitor=c("prediction.NBx"))
...
pred.NBerrx<-summary(as.mcmc.list(samples.nb,vars="prediction.NBx"),quantiles=c(0.005,0.025,0.25,0.5,0.75,0.975, 0.995))
pred.NB2errx<-data.frame(galaxy=hcat$Galaxy, NGC=hcat$N_GC, NGC_err=hcat$N_GC_err, dfx=dfx,dfz=dfz,mean=pred.NBerrx$statistics[,1],lwr1=pred.NBerrx$quantiles[,3],lwr2=pred.NBerrx$quantiles[,2],lwr3=pred.NBerrx$quantiles[,1],upr1=pred.NBerrx$quantiles[,5],upr2=pred.NBerrx$quantiles[,6],upr3=pred.NBerrx$quantiles[,7])
