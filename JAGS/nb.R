...
for (i in 1:N){
  xtrue[i] ~ dunif(xtmin,xtmax)# True predictor
}
...
inits<-function(){list(beta.0=rnorm(1,0,0.1),beta.1=rnorm(1,0,0.1),size=runif(1,0.1,50))}

# One for each chain
inits1 <- inits()
inits2 <- inits()
inits3 <- inits()
#params <- c("beta.0","beta.1","size","PRes","prediction.NB","xtrue","Fit","New")
...
# jagssamples.nb<-extend.jags(jags.neg, summarise=FALSE, add.monitor="prediction.NBx", drop.monitor=c("beta.0","beta.1","size"))
...
# pred.NBerrx<-summary(as.mcmc.list(jagssamples.nb,vars="prediction.NBx"),quantiles=c(0.005,0.025,0.25,0.5,0.75,0.975, 0.995))
...
# 
# g1<-ggs_density(s.nb3)+
#   theme_fivethirtyeight()+
#   theme(plot.background = element_rect(fill = 'white', colour = 'white')
# CairoPDF("Figures/posterior_densLK.pdf",height=10,width=8)
