...
GCS$density_err <- 0.10 * GCS$density #assume 10% error on density
...
preds <- c("mass","R_e","L_K","L_V","density")
...
for (p in preds) {
  for (i in preds) {
    ...
    df$up_x <- GCS[[p_err]]
    df$z <- GCS[[i]]
    i_err <- paste(i, "_err", sep="")
    df$up_z <- GCS[[i_err]]
    print(paste("Working on",p,"and",i))
    cl <- makeCluster(3)
    if(p == i){
    ...
    rm(jagssamples.nb)
    rm(jags.DIC)
    save(summaries, file='NB2summaries_autorun.Robj')
...
save(summaries, file='NB2summaries_autorun.Robj')
#save(outputs, file='Alloutputs_autorun.Robj')
save(Dstats, file='NB2dstats_autorun.Robj')
...
p <- ggplot(Dstats, aes(x=pred, y=DIC, fill=pred)) +
    ...
    labs(x="", y="", title=as.expression( bquote( "DIC"~"of"~N[GC]~"Predictors  N ="~.(nrow(GCS)) ))) +
    theme(axis.text.x =
