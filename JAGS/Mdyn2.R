#dim(GCS)
N_err<-GCS$N_GC_err
err_mass<-GCS$Mdyn_err
err_Mdyn<-GCS$Mdyn_err

N = nrow(GCS)

######## NB with errors ########################################################
massx = seq(from = 0.95 * min(GCS$Mdyn), 
...
mass_true[i] ~ dgamma(meanx^2/varx,meanx/varx)# True mass (predictor)
mass[i]~dnorm(mass_true[i],1/err_mass[i]^2)# Observed mass
...
# g2<-ggplot(GCS,aes(x=Mdyn,y=N_GC))+
...
#   geom_line(data=pred.NB2errx,aes(x=massx,y=mean),colour="gray25",linetype="dashed",size=0.8)+
...
# CairoPDF(paste(datapath,'Mdyn.pdf',sep=''),height=8,width=9)
# g2
# dev.off()
