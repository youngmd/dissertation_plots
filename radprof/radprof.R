...
rp <- subset(rp, r < max(outer, (2.0*extent)))
rp$Type = label1

rp2 <- rp[c("r","sd","sderr","Type")]
cnames <- c("r","sd","sderr")
sup <- tryCatch({
        read.table(paste(datapath,'/extra.radprof',sep=''), col.names=cnames)
      },
  warning = function(w){
    print()
  },
  error = function(err){
    data.frame(r=list(),sd=list(),sderr=list())
  }
)
sup$Type = label2
#sup$sd <- sup$sd * 0.9
...
g1 <- ggplot(data=rp) +  
    ...
    geom_rect(xmin=0.65*outer, xmax=0.98*outer, ymin=0.52*upper, ymax=0.98*upper, fill='white', color='black', alpha=0.95) +
    ...
    annotate("text", x = 0.77*outer, y = 0.64*upper, label = label1, parse=TRUE, size = 3, family="CM Roman", hjust = 0, vjust = 0.3) + 
    annotate("text", x = 0.77*outer, y = 0.56*upper, label = label2, parse=TRUE, size = 3, family="CM Roman", hjust = 0, vjust = 0.3)
