...
dist$R <- dnorm(dist$BmR,mean=m$parameters$mean[2], sd=sqrt(m$parameters$variance$sigmasq)) * (m$parameters$pro[2] * length(x) * binw)
...
g3 <- ggplot() + 
  ...
  xlab(expression('B - R')) + 
  annotate("text", x = 1.4, y = 0.9*max(dist$B,dist$R), label="Milky Way", size =4, hjust=0, vjust=0.5) +
  scale_y_continuous(expand= c(0,0)) 


m2 <- normalmixEM(mwgc$BR, mu=m$parameters$mean, maxit=10000, lambda=m$parameters$pro, sigma=sqrt(m$parameters$variance$sigmasq), k=2, mean.constr=c(NA, NA), arbmean=TRUE, arbvar=FALSE)
ad.score <- ADGofTest::ad.test(mwgc$BR, pnormmix, m2)
adboot <- bootstrapAD2(mwgc$BR, m2, arbvar=FALSE)
...
ggsave('MW_mm.pdf', gplot, width=8, height=8)
